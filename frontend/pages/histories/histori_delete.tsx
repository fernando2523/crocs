...
export default function Refund() {
    ...
    const [total_pesanan, setlist_delete_pesanan]: any = useState([]);
    const [total_pcs, setlist_delete_pcs]: any = useState([]);
    const [capital_amount, setcapitalamount] = useState(0);

    async function getdatarefund(tanggal: any, Store: any, Role: any, area: any, Query: any, Brand: any) {
        await axios
            .post("https://api.gudangsandal.com/v1/getdeleteorder", {
                tanggal: tanggal,
                store: Store,
                role: Role,
                area: area,
            ...
            .then(function (response) {
                // handle success
                setlist_refund(response.data.result.getdeleteorder);
                setlist_delete_pesanan(response.data.result.hasilpesanan);
                setlist_delete_pcs(response.data.result.hasilpcs);
    ...
    useEffect(() => {
        getdatarefund(date, Store, Role, area, Query, Brand);
            ...
            return list_produk.push(
                <div key={index}>
                    <div className="mb-2  group hover:shadow-lg ">
                        <div className="bg-white flex flex-row h-[40px] rounded-lg items-center px-0 text-[12px]">
    ...
    const handleValueChange = (newValue: any) => {
        if (newValue.startDate === null || newValue.endDate === null) {
            ...
            getdatarefund(startDate + " to " + lastDate, Store, Role, area, Query, Brand);
        } else {
            setDate(newValue.startDate + " to " + newValue.endDate);
            getdatarefund(newValue.startDate + " to " + newValue.endDate, Store, Role, area, Query, Brand);
        }
        setValues(newValue);
    };
    ...
    function querySet(e: any) {
        if (e.target.value === "") {
            setQuery("all");
            getdatarefund(date, Store, Role, area, "all", Brand);
        } else {
            setQuery(e.target.value);
            getdatarefund(date, Store, Role, area, e.target.value, Brand);
              ...
              <fa.FaSearch size={17} className="text-gray-700" />
                ...
                <select
                    ...
                    onChange={(e) => {
                        ...
                        getdatarefund(date, e.target.value, Role, area, Query, Brand);
                    }}
